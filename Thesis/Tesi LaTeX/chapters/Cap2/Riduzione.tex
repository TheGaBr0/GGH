...
\mathcal{H}(\mathbf{B}) = 
    \Biggl( 
        \frac{\det(\mathcal{L})}
        {\|\mathbf{b_1}\|_2 \cdot \|\mathbf{b_2}\|_2, \cdots \|\mathbf{b}_n\|_2} 
    \Biggr)^{\frac{1}{n}}
\]
dove $\|\cdot\|_2$ rappresenta la Norma Euclidea di $\cdot$.\\
...
\\
Prima di discutere questo tipo di algoritmi però è necessario affrontare brevemente 
...
viene utilizzata in algoritmi di riduzione dei reticoli. Come dimostrato in~\cite{HDMRD08},
teorema 7.13:
...
\begin{algorithm}
    ...
    \large
    \DontPrintSemicolon
    \KwIn{Una matrice $\mathbf{B}$ tale che $\text{rk}(\mathbf{B}) = \text{col}(\mathbf{B})$}
...
considerato una soluzione approssimata del apprSVP con un fattore di $2^{(n-1)/2}$ rispetto 
...
\begin{exmp} (Riduzione di una base con LLL)
    ...
    perciò $\mathbf{B}$ e $\mathbf{B}'$ sono entrambe basi valide per $\mathcal{L}$.
\end{exmp}
...
\\
L'ultima variante di LLL è basata sull'algoritmo di riduzione 
